...
#   ╚═════╝ ╚══════╝╚═╝  ╚═══╝   ╚═╝    ╚═════╝  ╚═════╝
#
#  Gentoo Interactive Installer  v3.0
...
read_password() {
  ...
  while true; do
    p1=$(dialog --title "Password" --passwordbox "$prompt"       9 52 3>&1 1>&2 2>&3) \
      || die "Password cancelled."
    p2=$(dialog --title "Password" --passwordbox "Confirm:"      9 52 3>&1 1>&2 2>&3) \
      || die "Password cancelled."
    if [[ "$p1" == "$p2" && -n "$p1" ]]; then
      printf -v "$varname" '%s' "$p1"
      return
    fi
    dialog --title "Error" --msgbox "Passwords do not match or are empty." 7 42
  done
}
...
# ──────────────────────────────────────────────
screen_welcome() {
  dialog --title "  Gentoo Installer v3.0  " --colors --msgbox "\n\
...
# Generated by Gentoo Installer v3.0
...
useradd -m -G users,wheel,audio,video,usb,plugdev,input -s /bin/bash "${USERNAME}" 2>/dev/null || \
  usermod -aG wheel,audio,video,usb,plugdev,input "${USERNAME}"
...
main() {
  ...
  screen_disk
  screen_filesystem
  # Ask swap only in auto-partition mode (manual sets BOOT_PART early)
  ...
  screen_stage3
  screen_keywords
  screen_march         # shows detected CPU name — fix #5
  screen_gpu           # VIDEO_CARDS dialog — fix #4
  screen_profile
  screen_use_flags
  screen_network
  screen_de
  ...
  # ── Installation ──
  dialog --title "Installing Gentoo" --infobox "\nPartitioning $DISK …" 6 46
  do_partition

  dialog --title "Installing Gentoo" --infobox "\nFormatting partitions …" 6 46
  do_format

  dialog --title "Installing Gentoo" --infobox "\nMounting filesystems …" 6 46
  do_mount

  do_stage3

  dialog --title "Installing Gentoo" --infobox "\nWriting make.conf …" 6 46
  do_makeconf

  dialog --title "Installing Gentoo" \
    --infobox "\nEntering chroot — this takes a while.\nMonitor: tail -f $LOG_FILE" 8 58
  do_chroot_setup
