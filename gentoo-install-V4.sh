...
do_partition() {
  ...
  # Let the kernel + udev settle on the new partition table
  udevadm settle 2>/dev/null || true
  sleep 2
  # Force re-read of partition table (important in VMs)
...
do_format() {
  ...
  # Let udev register the new filesystems before mounting (critical in VMs)
  udevadm settle 2>/dev/null || true
  sleep 2
  ok "Formatting done."
}
