...
#   ╚═════╝ ╚══════╝╚═╝  ╚═══╝   ╚═╝    ╚═════╝  ╚═════╝
#
#  Gentoo Interactive Installer  v4.0
#  Inspired by Arch & Void installers
#  License: MIT
#
#  Merged from v3 (your fixes) + v4 (new features):
#   #1  NVMe/eMMC partition naming (nvme0n1p1 not nvme0n11)
#   #2  Btrfs subvolumes  (@, @home, @cache, @snapshots)
#   #3  ZFS & Bcachefs filesystem options
#   #4  GPU / VIDEO_CARDS dialog
#   #5  CPU detection — show real CPU name before asking march
# ============================================================
...
NET_TOOL=""
DE_CHOICE=""
VIDEO_CARDS=""        # new in v3
LOCALE="en_US.UTF-8"
LOG_FILE="/tmp/gentoo-install.log"

# ──────────────────────────────────────────────
# FIX #1 — NVMe / eMMC partition helper
# nvme0n1  → nvme0n1p1
# sda      → sda1
# mmcblk0  → mmcblk0p1
# ──────────────────────────────────────────────
...
check_deps() {
  local missing=()
  for cmd in dialog parted mkfs.ext4 mkfs.fat wget tar wipefs partprobe blkid numfmt lspci; do
...
read_password() {
  ...
  # Loops until: non-empty, ≥8 chars, both entries match
...
# ──────────────────────────────────────────────
screen_welcome() {
  dialog --title "  Gentoo Installer v4.0  " --colors --msgbox "\n\
...
\ZbFeatures:\Zn\n\
  ...
  • NetworkManager / iwd / dhcpcd / ConnMan\n\
  • Optional Desktop Environment install\n\n\
\Z3Log:\Zn $LOG_FILE\n\
\Z1Minimum 25 GB free disk space required.\Zn" 24 64
}
...
# ──────────────────────────────────────────────
# SCREEN: FILESYSTEM  (fix #3 — ZFS + Bcachefs)
# ──────────────────────────────────────────────
screen_filesystem() {
  FS_TYPE=$(dialog --title "Root Filesystem" \
    --menu "Choose root filesystem:" 18 62 7 \
  ...
  # Warn for advanced filesystems
  case "$FS_TYPE" in
    zfs)
      dialog --title "ZFS Warning" --colors --msgbox \
        "\n\Z3ZFS requires \Zbsys-fs/zfs\Zn to be installed in the Live environment first.\Zn\n\n\
...
screen_swap() {
  SWAP_SIZE=$(dialog --title "Swap" \
    --inputbox "Swap partition size (e.g. 4G, 8G). Enter 0 to skip:" \
    9 50 "4G" 3>&1 1>&2 2>&3) || die "Swap cancelled."
  info "Swap: $SWAP_SIZE"
}

# ──────────────────────────────────────────────
# SCREEN: HOSTNAME / USERNAME
# ──────────────────────────────────────────────
screen_hostname() {
...
screen_username() {
  USERNAME=$(dialog --title "User Account" \
    --inputbox "Your username (lowercase):" 9 46 \
...
screen_passwords() {
  read_password ROOT_PASS "Root password:"
  read_password USER_PASS "Password for ${USERNAME}:"
}
...
screen_stage3() {
  ...
  # Parse: first non-comment line starting with "stage3-"
...
# ──────────────────────────────────────────────
# SCREEN: CPU DETECTION + MARCH  (fix #5)
# ──────────────────────────────────────────────
screen_march() {
  # Detect CPU name from /proc/cpuinfo
  local CPU_NAME
...
screen_gpu() {
  ...
  # Extra note for NVIDIA proprietary
...
screen_de() {
  DE_CHOICE=$(dialog --title "Desktop Environment" \
    --menu "Install a Desktop Environment?" 16 64 6 \
...
# ──────────────────────────────────────────────
screen_summary() {
  dialog --title "Installation Summary" --colors \
    --yesno "\
\ZbDisk:\Zn          $DISK  \Z1(WILL BE ERASED)\Zn\n\
...
\ZbUSE flags:\Zn     ${USE_FLAGS:-<defaults>}\n\n\
\Z1Proceed? Disk will be wiped — no undo.\Zn" \
    26 72 || die "Installation cancelled by user."
}

# ──────────────────────────────────────────────
# INSTALL: PARTITION  (uses part() helper — fix #1)
# ──────────────────────────────────────────────
do_partition() {
  # Skip if manual mode already populated BOOT_PART
  [[ -n "$BOOT_PART" ]] && { info "Using manually defined partitions."; return; }

  info "Auto-partitioning $DISK …"

  # ── Clean up any leftovers before partitioning ──
  info "Cleaning up $DISK before partitioning …"
  ...
  # Convert swap size to MiB for parted arithmetic
  local SWAP_MiB=0
  if [[ "$SWAP_SIZE" != "0" ]]; then
    local raw_bytes
    raw_bytes=$(numfmt --from=iec "$SWAP_SIZE" 2>/dev/null) || raw_bytes=$((4 * 1024 * 1024 * 1024))
...
# ──────────────────────────────────────────────
# INSTALL: FORMAT  (fix #2 — Btrfs subvols; fix #3 — ZFS/Bcachefs)
# ──────────────────────────────────────────────
do_format() {
  ...
  case "$FS_TYPE" in
    ...
    btrfs)
      ...
      umount /mnt/gentoo
      ok "Btrfs subvolumes created: @  @home  @cache  @snapshots"
      ;;
    xfs)
    ...
    zfs)
      ...
      ok "ZFS pool '$POOL' created with root + home datasets."
      ;;
  esac
...
do_mount() {
  ...
  if [[ "$FS_TYPE" == "btrfs" ]]; then
    ...
    : # ZFS already mounted by zpool/zfs create above
  else
    mount "$ROOT_PART" /mnt/gentoo
  fi
...
# Generated by Gentoo Installer v4.0
...
# ──────────────────────────────────────────────
# INSTALL: FSTAB GENERATOR  (Btrfs subvols aware)
# ──────────────────────────────────────────────
_gen_fstab() {
...
do_chroot_setup() {
  ...
  # Write fstab from host (we have blkid access)
  _gen_fstab > /mnt/gentoo/etc/fstab
  info "fstab written."

  # Build network / DE package strings
  local NET_PKG
  case "$NET_TOOL" in
...
case "${KERNEL_CHOICE}" in
  ...
  manual)
    ...
    log "Kernel sources installed. Configure manually post-reboot."
    ;;
esac
...
# Create plugdev group if it doesn't exist (not present in base Gentoo)
getent group plugdev &>/dev/null || groupadd plugdev >> "\$LOG" 2>&1 || true
useradd -m -G users,wheel,audio,video,usb,input -s /bin/bash "${USERNAME}" 2>/dev/null || \
  usermod -aG wheel,audio,video,usb,input "${USERNAME}"
# Add to plugdev separately after ensuring it exists
...
if [[ "${BOOT_MODE}" == "uefi" ]]; then
  grub-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=Gentoo >> "\$LOG" 2>&1
else
...
CHROOT_EOF
  ...
  chroot /mnt/gentoo /usr/bin/env \
    ROOT_PASS_IN="$ROOT_PASS" USER_PASS_IN="$USER_PASS" \
...
\ZbNext steps:\Zn\n\
  1. \Z6reboot\Zn\n\
  2. \Z6emerge --sync && emerge -uDN @world\Zn\n\
  3. Install additional software as needed\n\n\
\Z3Full log:\Zn $LOG_FILE\n\n\
\ZbWelcome to Gentoo!\Zn — compile wisely 🐧" 22 60
}

# main() defined below in v4.0 ADDITIONS block

# ══════════════════════════════════════════════
#  v4.0 ADDITIONS  (state/resume + mirror + live preview + progress bars)
# ══════════════════════════════════════════════
...
GENTOO_MIRROR="https://distfiles.gentoo.org"   # default, overridden by screen_mirror
...
# ──────────────────────────────────────────────
# PROGRESS BAR ENGINE
# animate gauge from $1→$2 while PID $3 is alive
# ──────────────────────────────────────────────
_gauge_anim() {
  ...
  case "$method" in
    ...
    custom)
      GENTOO_MIRROR=$(dialog --title "Custom Mirror" \
        --inputbox "Full mirror URL:" 9 70 "https://distfiles.gentoo.org" \
        3>&1 1>&2 2>&3) || die "Mirror cancelled."
      ;;
  esac
...
_mirror_auto() {
  ...
  for url in "${!M[@]}"; do
    ...
    if [[ "$ms" -lt "$best_ms" ]]; then best_ms=$ms; best_url=$url; fi
  done
...
_mirror_by_region() {
  ...
  region=$(dialog --title "Mirror Region" \
    --menu "Choose your region:" 20 58 9 \
...
# ──────────────────────────────────────────────
# OVERRIDE main() — v4 version with all features
# ──────────────────────────────────────────────
main() {
  require_root
  mkdir -p /mnt/gentoo
...
# Helper called from gauge_phase for root format
_do_format_root() {
  case "$FS_TYPE" in
    ext4)     mkfs.ext4 -F "$ROOT_PART" ;;
    xfs)      mkfs.xfs  -f "$ROOT_PART" ;;
    f2fs)     mkfs.f2fs -f "$ROOT_PART" ;;
    bcachefs) mkfs.bcachefs --force "$ROOT_PART" ;;
    btrfs)
    ...
    zfs)
      zpool create -f -o ashift=12 \
        -O compression=lz4 -O atime=off -O xattr=sa -O mountpoint=none \
        rpool "$ROOT_PART"
      zfs create -o mountpoint=/mnt/gentoo rpool/root
